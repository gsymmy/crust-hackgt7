...
function getEntitlementsPath(projectRoot) {
    const [using, ...extra] = glob_1.sync('ios/*/.entitlements', {
        absolute: true,
        cwd: projectRoot,
        ignore: ignoredPaths,
    });
...
function warnMultipleFiles({ tag, fileName, using, extra, }) {
    WarningAggregator_1.addWarningIOS(`paths-${tag}`, `Found multiple ${fileName} file paths, using "${using}". Ignored paths: ${JSON.stringify(extra)}`);
}
